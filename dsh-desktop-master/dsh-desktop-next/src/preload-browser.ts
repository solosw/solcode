...
export function createDesktopBrowserBridge(): DesktopBrowserBridge {
  ...
  ipcRenderer.on(IPC.browserOpenRequested, (_event, request: unknown) => {
  ...
  return {
    acquire: workspace => ipcRenderer.invoke(IPC.browserAcquire, workspace) as ReturnType<DesktopBrowserBridge['acquire']>,
    release: lease => ipcRenderer.invoke(IPC.browserRelease, lease) as Promise<void>,
